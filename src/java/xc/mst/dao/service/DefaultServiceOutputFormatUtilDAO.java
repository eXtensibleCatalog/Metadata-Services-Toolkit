	...
	public List<Integer> getOutputFormatsForService(int serviceId)
	{
		synchronized(psGetOutputFormatsForServiceLock)
		{
			...
			try
			{
				...
				if(psGetOutputFormatsForService == null)
				{
					// SQL to get the rows
					...
					// This should sanitize the SQL and prevent SQL injection
					psGetOutputFormatsForService = dbConnection.prepareStatement(selectSql);
				}
				...
				// Execute the query
				results = psGetOutputFormatsForService.executeQuery();
	...
	public boolean deleteOutputFormatsForService(int serviceId)
	{
		synchronized(psDeleteOutputFormatsForServiceLock)
		{
			...
			try
			{
				...
				if(psDeleteOutputFormatsForService == null)
				{
					// SQL to get the rows
					...
					// A prepared statement to run the select SQL
					// This should sanitize the SQL and prevent SQL injection
					psDeleteOutputFormatsForService = dbConnection.prepareStatement(selectSql);
				}
				...
				// Execute the insert statement and return the result
				return psDeleteOutputFormatsForService.executeUpdate() > 0;
